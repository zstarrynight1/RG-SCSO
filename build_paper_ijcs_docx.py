...
ASOC_CITE_ORDER = [
    ...
    "bgoatvg2024", "fhsbpso2024", "mobgwogms2023", "hhoelm2023",
    "whalehho2022", "xaifs2024",
]
...
def _msz(text, half=16):
    r = M("r")
    rpr = OxmlElement("w:rPr")
    for tag in ("w:sz", "w:szCs"):
        e = OxmlElement(tag); e.set(qn("w:val"), str(half)); rpr.append(e)
    r.append(rpr)
...
def _cell_math(cell, elements):
    oMath = M("oMath")
    oMath.extend(elements)
    cell.paragraphs[0]._p.append(oMath)


def _notation_omath(sym):
    R = _msz
    def S(b, s): return msub([R(b)], [R(s)])
    table = {
        "d": [R("d")],
        "b ∈ {0,1}ᵈ": [R("b ∈ "), msup([R("{0,1}")], [R("d")])],
        "f(b)": [R("f"), mdelim([R("b")])],
        "Acc(b)": [R("Acc"), mdelim([R("b")])],
        "|b|": [mdelim([R("b")], "|", "|")],
        "λ": [R("λ")],
        "R(t)": [R("R"), mdelim([R("t")])],
        "S_M": [S("S", "M")],
        ...
        "σⱼ": [S("σ", "j")],
        "pⱼ": [S("p", "j")],
        "V(xⱼ)": [R("V"), mdelim([S("x", "j")])],
        "τ": [R("τ")],
        "ρ_static": [S("ρ", "static")],
        "I(Xⱼ;y)": [R("I"), mdelim([S("X", "j"), R(";y")])],
        "H(y)": [R("H"), mdelim([R("y")])],
        "K": [R("K")],
        "N": [R("N")],
        "max_nfe": [R("max_nfe")],
...
def add_notation_table(doc) -> None:
    ...
    for sym, meaning in _NOTATION_ROWS:
        cells = t.add_row().cells
        elems = _notation_omath(sym)
        if elems is not None:
            _cell_math(cells[0], elems)                                        
        else:
...
def _alg_math_omml(expr):
    R = lambda t: _msz(t, 17)                           

    def match_close(s, i, opench, closech):
        depth = 0
        while i < len(s):
            if s[i] == opench:
                depth += 1
    ...
    def parse_script(s, i):
        if i < len(s) and s[i] == "{":
            j = match_close(s, i, "{", "}")
            return parse(s[i + 1:j]), j + 1
        m = re.match(r"[^\W_]+", s[i:])
        if m:
            return [R(m.group(0))], i + m.end()
        return [R(s[i])], i + 1
    ...
    def parse(s):
        ...
        while i < len(s):
            ...
            if i < len(s) and s[i] == "_":
                sub, i = parse_script(s, i + 1)
            if i < len(s) and s[i] == "^":
                sup, i = parse_script(s, i + 1)
            if sub is not None and sup is not None:
                elems.append(msub([msup(base, sup)], sub))
            elif sub is not None:
                elems.append(msub(base, sub))
            elif sup is not None:
                elems.append(msup(base, sup))
            else:
                elems.extend(base)
        return elems
...
def add_algorithm_box(doc) -> None:
    ...
    lines = [
        ...
        "3:  initialize positions $x_i$ ~ $U(−1,1)^d$, $i = 1..N$; form the "
        "initial masks by sign only (bit=1 if $x>0$, else 0); evaluate; set "
        "$b^*$   ▷ the relevance-modulated flip rule (lines 8–11) is not "
    ...
    for idx, line in enumerate(lines):
        p = acell.paragraphs[0] if idx == 0 else acell.add_paragraph()
...
def add_classic_baselines_all18(doc):
    from build_paper_ijcs import _classic18_data
    d = _classic18_data()
    cols = ["Dataset"] + d["columns"]
    t = doc.add_table(rows=1, cols=len(cols))
    _ieee_table(t)
    t.alignment = WD_TABLE_ALIGNMENT.CENTER
    t.autofit = True
    ...
    for ds in d["datasets"]:
        ...
        for j, c in enumerate(d["columns"]):
            ...
            run = cells[1 + j].paragraphs[0].add_run(f"{a:.3f} ({n:.0f})")
            run.font.size = Pt(7)
            if abs(a - best) < 1e-9:
                run.bold = True
    widen_first_col(t, 1.0)
    rel = d["wl"].get("ReliefF", {})
    caption(doc, "Table 6 Full 18-dataset comparison of RG-SCSO against five "
                 "classical filter/embedded/wrapper selectors and a "
                 "Concrete-Autoencoder deep feature selector (CAE); accuracy "
                 "with mean selected-feature count in parentheses, 30 runs, "
                 "identical stratified 5-fold KNN evaluator, bold = best "
                 "accuracy per dataset. RG-SCSO has the best accuracy of all "
                 f"methods on {d['best_all']} of {d['n']} datasets and is "
                 "significantly more accurate (Holm-corrected Wilcoxon over "
                 "the 18 datasets) than MI-thresholding, SFS, mRMR, and LASSO; "
                 f"the one classical method it does not significantly beat is "
                 f"ReliefF (RG better on {rel.get('wins', 13)}/{d['n']}, "
                 f"p={rel.get('holm_p', 0.20):.2g}). The CAE, matched to "
                 "RG-SCSO's per-dataset subset size, is the weakest selector "
                 "on these small-sample datasets. LASSO and mRMR lead on the "
                 "two gene-expression (p >> n) sets.")


def add_component_ablation_table_ijcs(doc, s):
    ...
    caption(doc, "Table 4 Component ablation: mean accuracy over 30 runs on "
                 "five datasets. † = significantly worse than Full (paired "
                 "Wilcoxon signed-rank, Holm-corrected p < 0.05). ‘− ORL’ is "
                 "the final RG-SCSO (RMS+UMR); ORL removal never degrades "
                 "accuracy, so ORL is not retained. The signal-position "
                 "experiment (alternative relevance-injection points) is "
                 "reported separately in the Supplementary Information.")
...
def build() -> None:
    ...
    kwr = kw.add_run("Feature selection, Sand Cat Swarm Optimization, binary "
                      "optimization, relevance-guided binarization, "
                      "parsimony.")
    kwr.italic = True
    kwr.font.size = Pt(9)
    ...
    para(doc, "We close this gap with RG-SCSO: a per-feature, "
              ...
              "we propose RG-SCSO, whose ablation-confirmed centerpiece is "
              "relevance-modulated sensitivity (RMS), supplemented by a "
              "smaller, budget-neutral memetic refinement step (UMR) that a "
              ...
              "parsimony with relevance guidance.")
    ...
    doc.add_heading("2.1 Binary transfer functions in swarm-based feature "
                     "selection", level=2)
    para(doc, f"Swarm-intelligence wrapper selectors, including grey wolf "
              ...
              "optimization and cross into the binary search space through a "
              f"transfer function, most commonly an S-shaped or V-shaped map "
              f"{_c('tf')}. A substantial recent line of work makes this "
              "interface adaptive rather than static: time-varying transfer "
              "functions that reshape the squashing curve as the search "
              f"progresses {_c('islam2017tvtf','bgoatvg2024')}, and fitness- "
              "or history-driven schemes that tune it from the swarm's own "
              f"progress {_c('teng2017avbpso','fhsbpso2024')}. Crucially, all "
              "of these remain feature-agnostic: the same (possibly "
              "time-varying) transfer is applied identically to every "
              "dimension regardless of that feature's relevance, the "
              "structural weakness this paper terms washout (Introduction, "
              "Methods). RG-SCSO is orthogonal to this line: it modulates the "
              "binarization per feature by a relevance prior, and could in "
              "principle be combined with an adaptive transfer rather than "
              "replacing one.")
    doc.add_heading("2.2 Relevance-guided mechanisms in feature selection",
                     level=2)
    para(doc, f"Filter criteria such as mutual information and mRMR "
              f"{_c('guyon','mrmr')} encode problem knowledge cheaply but are "
              f"decoupled from the wrapper search; memetic hybridization "
              f"{_c('neri')} adds local refinement without addressing the "
              "binarization interface. A large family of knowledge-guided "
              "metaheuristics injects filter information upstream of the "
              "binary decision, into initialization or the objective. In "
              "other swarm families this takes the form of correlation- or "
              "mutual-information-guided population initialization "
              f"{_c('mobgwogms2023','hhoelm2023')}, filter-guided PSO for "
              f"cancer-genome selection {_c('ludwig2025guided')}, and memetic "
              "hybridization of binary Harris-hawks and whale operators "
              f"{_c('whalehho2022')}; a parallel, deep-learning-oriented "
              "direction drives selection from post-hoc attribution or "
              f"attention, for example SHAP-guided deep feature selection "
              f"{_c('xaifs2024')}. All of these bias where the search starts, "
              "what it optimizes, or which features a separately trained "
              "model highlights, yet leave the binarization operator itself "
              "knowledge-agnostic. RG-SCSO differs by injecting the relevance "
              "signal at the binarization interface itself rather than "
              "upstream of it; the signal-position experiment in Results "
              "(Table 4) tests this design choice directly against both the "
              "initialization and objective alternatives.")
    _p = doc.add_paragraph()
    _p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    _p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    _p.paragraph_format.space_after = Pt(10)
    _lead = _p.add_run("Positioning against relevance-injecting binary "
                       "GWO/WOA/HHO variants. ")
    _lead.bold = True
    _lead.font.size = Pt(BODY_PT)
    _rest = _p.add_run(
        "The closest competitors are binary grey-wolf, whale, and "
        "Harris-hawks selectors that already carry a relevance signal: "
        f"MOBGWO-GMS initializes its population from feature correlation and "
        f"guides mutation with it {_c('mobgwogms2023')}, the enhanced "
        f"Harris-hawks optimizer seeds the search with filter-ranked features "
        f"{_c('hhoelm2023')}, and hybrid binary whale/Harris-hawks memetics "
        f"combine two operators for stronger local search {_c('whalehho2022')}. "
        "Each injects relevance at initialization, mutation, or "
        "hybridization; none makes the per-feature binarization operator "
        "itself relevance-aware, which is the specific interface RG-SCSO "
        "modifies and the reason its parsimony gain (Results) is not "
        "reproduced by these variants' upstream injections.")
    _rest.font.size = Pt(BODY_PT)
    doc.add_heading("2.3 Sand Cat Swarm Optimization and its variants", level=2)
    ...
    doc.add_heading("3.1 Held-out generalization", level=2)
    ...
    doc.add_heading("3.2 Feature-subset parsimony", level=2)
    inference_sentence = ""
    if inf_val is not None:
        inference_sentence = (
            ...
            "above call overhead; a projected, reconstructed inference-cost "
            "analysis at a synthetic deployment-scale workload, a "
            "controlled complexity demonstration rather than a measurement "
            "on the benchmark's own test sets, is reported in Supplementary "
            "Information instead of here.")
    ...
    doc.add_heading("3.3 Ranking and statistical significance", level=2)
    if s.get("stats"):
    ...
    doc.add_heading("3.4 Ablation and mechanism", level=2)
    if s.get("ablation"):
        para(doc, "We started from a three-component design and tested "
                  ...
                  "RMS and UMR only.")
        full_width(doc, lambda: add_component_ablation_table_ijcs(doc, s))
    ...
    para(doc, "RQ3 asks whether direct relevance injection at the "
              ...
              "the binarization interface; both alternatives are reported "
              "in full in the Supplementary Information. MI-weighted "
              "objective injection "
    ...
    doc.add_heading("3.6 Comparison with classical selectors", level=2)
    para(doc, "RQ4 asks how robust the mechanism is across classifiers, "
              "relevance priors, and dataset dimensionalities; this "
              "subsection and Table 7 answer it directly. Table 6 compares "
              "RG-SCSO across the full 18-dataset benchmark against five "
              "classical filter, embedded, and wrapper selectors, "
              "mutual-information thresholding, mRMR, ReliefF, LASSO, and "
              "sequential forward selection, and a Concrete-Autoencoder deep "
              "feature selector, under the identical stratified 5-fold KNN "
              "evaluator. RG-SCSO attains the best accuracy of all methods on "
              "12 of 18 datasets. Across the benchmark it is significantly "
              "more accurate (Holm-corrected Wilcoxon signed-rank over the 18 "
              "datasets) than mutual-information thresholding, sequential "
              "forward selection, mRMR, and LASSO (all Holm p < 0.05); the "
              "one classical selector it does not significantly beat is "
              "ReliefF, which it still leads on 13 of 18 datasets (p = 0.20). "
              "The Concrete-Autoencoder baseline, matched to RG-SCSO's "
              "per-dataset subset size, is the weakest selector on the "
              "small-sample datasets tested, consistent with deep "
              "reconstruction objectives needing far more than the 50-455 "
              "training instances these benchmarks provide. The exceptions to "
              "RG-SCSO's lead are informative and concentrated: on the two "
              "gene-expression (p >> n) datasets a classical LASSO baseline "
              "attains higher accuracy with far fewer features on both "
              ...
              "method edges it by at most 1.7 accuracy points. RG-SCSO's "
              "advantage is thus broad but not universal: clearest on "
              "datasets without extreme p >> n structure, where on "
              "ultra-high-dimensional gene-expression data a computationally "
              "far cheaper embedded method such as LASSO is a strong, arguably "
              "preferable, alternative. A remaining question is whether this "
              "parsimony mechanism is an artifact of the KNN wrapper used "
              "throughout; Table 7 answers it directly, extending the same "
              "relevance-guided search to SVM and Random Forest wrappers. The "
              "advantage over a no-prior baseline is not a KNN artifact, "
              "though it is a more consistent parsimony gain than an accuracy "
              "one under Random Forest specifically, and the ReliefF-prior "
              "degradation already established above reproduces under every "
              "wrapper tested.")
    full_width(doc, lambda: add_classic_baselines_all18(doc))
    full_width(doc, lambda: (
        ...
        _renumber_caption(doc, {
            ...
            "as Table 4 (KNN/SVM": "(Zoo, Sonar, WDBC, ColonCancer, "
                                   "Leukemia; KNN/SVM",
        }),
    ))
    ...
    para(doc, "These results trace washout, a concrete failure mode of "
              ...
              "motivating this design (Methods) is a diagnostic bound: it "
              "explains why continuous-space enhancements fail at the "
              "binarization boundary, but it is not a convergence guarantee "
              "for RG-SCSO itself. The underlying recipe, a filter prior "
              ...
              "cross-classifier and cross-prior robustness, and an explicit "
              "exploration-safety diagnostic. We report parsimony, not raw "
              "accuracy, as the transferable outcome of this choice.")
    ...
    para(doc, "Several boundaries delimit what these results establish. "
              ...
              "confirming the mechanism: the parsimony advantage disappears "
              "entirely (Supplementary Information). RG-SCSO's compactness "
              "therefore depends on a prior that drives uninformative "
    ...
    para(doc, "The "
              ...
              "SVM check because an RBF-kernel SVM refit at every one of the "
              "wrapper's roughly 15,000 fitness evaluations per run is "
              "intractable within budget on the two largest-n datasets), and "
              "under Random Forest on the same five-dataset subset as "
              "Table 7. The parsimony advantage holds beyond KNN. Under "
    ...
    para(doc, "External validity has its own limits: the "
              ...
              f"Its top end ({feat_max} features, Leukemia) is still an order "
              "of magnitude below the 10^4-10^5-feature omics and "
              "high-dimensional text regimes where a binary-native, "
              "relevance-guided operator should matter most, so behaviour at "
              "that scale is extrapolated, not measured; a genuine "
              ">10^4-feature evaluation is deferred to future work because a "
              "full seven-optimizer, 30-run wrapper study at that "
              "dimensionality is compute-bound by the KNN fitness "
              "evaluations rather than by the relevance mechanism itself, and "
              "lies outside this study's fixed budget. The "
    ...
    para(doc, "This advantage is not universal, and the boundaries "
              ...
              "against adaptive-transfer or same-family SCSO baselines, and "
              "LASSO outperforms it outright on the two extreme p >> n "
              "gene-expression datasets. Its parsimony depends on the "
              "relevance prior's quality: a ReliefF prior removes the "
              "advantage entirely. The preferred-bit threshold, while "
              "empirically reasonable, is not uniformly optimal. And the "
    ...
    fw.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    fw.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    fw.paragraph_format.space_after = Pt(10)
    _add_run_text(fw, "Future work ", bold=True, size=BODY_PT)
    _add_run_text(fw, "includes: (i) adaptive, data-driven selection of the "
                ...
                "the prior used here or transfers to others.", size=BODY_PT)
    ...
    doc.add_heading("6.2 Theoretical motivation: transfer-function washout", level=2)
    ...
    doc.add_heading("6.3 The RG-SCSO mechanism", level=2)
    ...
    doc.add_heading("6.4 Algorithm and computational cost", level=2)
    ...
    doc.add_heading("6.5 Cardinality-weight sensitivity", level=2)
    ...
    doc.add_heading("6.6 Datasets, baselines, and protocol", level=2)
    ...
    para(doc, "Bui Quang Huy: Conceptualization, Methodology, Software, "
              ...
              "Visualization, Supervision. Duong Minh Son: Formal analysis, "
    ...
    para(doc, "Not applicable. This study uses only publicly available "
